...
export class StoryVersionCachingService<TContent = unknown> {
  ...
  fetchBySlug = async (slug: string): Promise<TContent | null> => {
    ...
    if (cachedCv !== null) {
      ...
      result = await this.contentClient.fetchBySlug(slug, cachedCv) as StoryblokResponse<TContent> | null;
    }
    ...
    if (result?.story) {
      ...
      result = await this.contentClient.fetchBySlug(slug) as StoryblokResponse<TContent> | null;
...
export function createStoryVersionCachingService<TContent = unknown>(): StoryVersionCachingService<TContent> {
  if (!_cachingService) {
    const cache = createSupabaseClient();
    const contentClient = createStoryblokClient();
    _cachingService = new StoryVersionCachingService<TContent>(cache, contentClient);
  }
