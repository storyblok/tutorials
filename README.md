# Tutorials

This repository includes all the code necessary to follow [tutorials](https://storyblok.com/tutorials) created by Storyblok's documentation and developer relations teams.

It serves as a central hub for learning materials, integration examples, and step-by-step tutorials designed to help developers explore and build with Storyblok.

[![Try Storyblok free](https://img.shields.io/badge/Try%20Storyblok-dad4ff.svg?style=for-the-badge&logo=data:image/svg+xml;base64,&labelColor=ffffff)](https://app.storyblok.com/#/signup?utm_source=docs)
[![Join the Storyblok Discord community](https://img.shields.io/discord/700316478792138842?style=for-the-badge&logo=discord&label=Join%20our%20community&labelColor=ffffff&color=dad4ff)](https://storyblok.com/join-discord)

## Get started

The projects in this repository are organized by topic, and follow a clear name pattern: `tech-keyword`. This means, that the code for the tutorial **Integrate Algolia Search into Storyblok with Astro** is found in `[search/astro-algolia](search/algolia)`, while the code for the tutorial **Build a GEO-ready website with Storyblok and Astro** is found in `[ai/astro-geo](ai/astro-geo)`.

Each tutorial is stored in a directory that includes a `README` file with setup instructions, links, and relevant information.

You can clone any tutorial and use it as a standalone project.

> [!TIP]
> **No Storyblok account yet? [Sign up now](https://www.storyblok.com?utm_source=github.com&utm_medium=readme&utm_campaign=tutorials) to experience a 14-day free trial of all features.**

## Tutorial list

- [Integrate Algolia Search into Storyblok with Astro](search/astro-algolia)
- [Master SEO with Storyblok and Astro](search/astro-seo)
- [Bulk update AI-generated meta fields](ai/node-update-ai-seo)
- [Build a GEO-ready website with Storyblok and Astro](ai/astro-geo)

## Additional resources

- [Storyblok documentation](https://storyblok.com/docs)
- [Storyblok developer tutorials](https://storyblok.com/tutorials)
- [Storyblok Help Center](https://support.storyblok.com/)
- [Storyblok SDKs](https://github.com/storyblok/monoblok)
- [Storyblok plugins](https://github.com/storyblok/pluginsblok)